                ...
                <div class="row">
                    ...
                    <div class="col-md-9  pr0 padding-top-40 properties-page">                    
                        <div class="col-md-12 clear"> 
                            <h4 class="center sectionTitle"><srtong>::</srtong> Bidding Summery <srtong>::</srtong> </h4>
                            <div class="col-xs-12">
                                    <?php
                            ...
                            <div class="hpp_wpapper_table">
                                     ...
                                     ?>
                                     <ul>
                                        <?php if($account_type == 1){?><li class="<?php if($get_search == 'auction'){$nae = 'Bid Win Price ';  echo 'active';}?>" ><a href="<?= SITE_URL ?>bidding-summery?search=auction" >Aucting Property </a></li> <?php }?>
                                        <?php if($account_type == 2){?><li class="<?php if($get_search == 'bidder'){$nae = 'Your Bid Price';  echo 'active';}?>" ><a href="<?= SITE_URL ?>bidding-summery?search=bidder" >Bidder</a></li> <?php }?>
										...
										<div class="table-responsive">
										 <table class="table table-striped table-bordered datatable">
												 ...
												 foreach ( $select_property_by_user as $uProperty ) {
												   ...
												   if($uProperty->OFFER_STATUS == 'Active'){
													   if($checkDate >= $time){
															...
															if($get_search == 'auction'){
																	$status = 'Date Over <br/> <a href="#" class=" mar-r-3" onclick="selectReAuctionModal('.$uProperty->PROPERTY_ID.','.$uProperty->OFFER_P_ID.')" data-toggle="modal" data-target="#reAuctionModal" data-whatever="@auctionmodal" title="Add Re-Auction">Re-Auction</a>';
																}else{
																	$status = 'Date Over';
																}          
													   } 
												   ...
												   if ($uProperty->OFFER_STATUS == 'Win' && $uProperty->WIN_USER > 0) {
														   if ($uProperty->WIN_USER == $this->userID) {
																   ...
																   if($biddingCount == 0){
																		$btnLink = '<a href="'.SITE_URL.'payment-now?id='.$this->Property_Model->encode_str($uProperty->PROPERTY_ID).'" class="btn btn-default mar-b-5" title="" >Pay Now</a><a href="#" data-toggle="modal" onclick="AddSolicitorsDetailsModal(' . $uProperty->PROPERTY_ID . ',' . $uProperty->OFFER_P_ID . ', 1)" data-target="#solicitorsModal" data-whatever="@solicitorsmodal" class="btn btn-default" title="" >Add Solicitors</a>';
																   }else{
																	   $btnLink = '<a href="'.SITE_URL.'payment-now?id='.$this->Property_Model->encode_str($uProperty->PROPERTY_ID).'" class="btn btn-default mar-b-5" title="" >Pay Now</a> <a href="#" data-toggle="modal" onclick="AddSolicitorsDetailsModalView(' . $uProperty->PROPERTY_ID . ',' . $uProperty->OFFER_P_ID . ', 1)" data-target="#solicitorsModalView" data-whatever="@solicitorsmodalView" class="btn btn-default" title="" >Solicitors</a>';
																   }
														   ...
														   $btnLink = '<a href="#" data-toggle="modal" onclick="AddOffrCounterModal(' . $uProperty->PROPERTY_ID . ',' . $uProperty->OFFER_P_ID . ', 1)" data-target="#counterModal" data-whatever="@countermodal" class="btn btn-default" title="" >Auction</a>';
												   }
											 ?>
											 <tr id="property_tr__<?= $uProperty->PROPERTY_ID; ?>">
												...
												<?php if($get_search == 'auction'){ ?> <td class="center"> <!--<a href="<?= SITE_URL ?>bidding-summery?search=auction&id=<?= $uProperty->PROPERTY_ID;?>&auction=<?= $uProperty->OFFER_P_ID;?>"><b> <?= number_format($totalBidder); ?> </b> </a> -->
													   <a href="#" data-toggle="modal" onclick="selectAuctionDetailsUserModal(<?= $uProperty->PROPERTY_ID;?>,<?= $uProperty->OFFER_P_ID;?>);" data-target="#hotPriceModal" data-whatever="@hotPriceModal" class="bidder_a" title="View Auction Info" ><b> <?= number_format($totalBidder); ?> </b></a>
												</td> <?php }?>
												<?php if($get_search == 'bidder'){ ?> <td class="center"> <b> <?= number_format($totalBidder); ?> </b></td> <?php }?>
												 
												 <td class="center"><?= $status; ?></td>
												<?php if($get_search == 'auction'){ ?> 
												 <td class="center"> <a href="#" data-toggle="modal" onclick="selectAuctionDetailsUserModal(<?= $uProperty->PROPERTY_ID;?>,<?= $uProperty->OFFER_P_ID;?>);" data-target="#hotPriceModal" data-whatever="@hotPriceModal" class="view_data btn btn-default mar-r-3" title="View Bidding Info" ><b> <i class="fa fa-eye"></i> </b></a></td> <?php }?>
...
<div class="modal fade" id="counterModal" role="submit Form Data" aria-labelledby="AddSolicitors" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content brr-0">
            <div class="modal-header">
                <h5 class="modal-title hppModalTitle" id="AddPropertToHotPriceSoli"> Auction Information </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
